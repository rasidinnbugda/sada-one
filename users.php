...
<div class="filtre-bar">
    <div class="arama-kutu"><svg fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><path d="M21 21l-4.3-4.3M17 11a6 6 0 11-12 0 6 6 0 0112 0z"/></svg><input class="girdi" placeholder="İsim veya e-posta ara..." data-search="#userList tr"></div>
    <div class="pill-filtre" data-pill-grup="#userList tr">
...
function roleDegisti() {
    ...
    if (roleDefault[role] && !window.permissionOzel) {
        document.querySelectorAll('.izin-kutu').forEach(c => { c.checked = !!roleDefault[role][c.dataset.setting_key]; });
    }
}
function userSifirla() {
    ...
    document.getElementById('passwordIpucu').textContent = 'En az 8 karakter.';
...
function userEdit(k) {
    ...
    document.querySelectorAll('.mdosya-kutu').forEach(c => { c.checked = selected.has(c.value); });
    ...
    document.querySelectorAll('.izin-kutu').forEach(c => { c.checked = !!taban[c.dataset.setting_key]; });
    roleDegisti();
    modalOpen('modalUser');
}
...
document.getElementById('userForm').addEventListener('submit', () => {
    ...
    if (roleDefault[role]) {
        const permissions = {};
        document.querySelectorAll('.izin-kutu').forEach(c => { permissions[c.dataset.setting_key] = c.checked ? 1 : 0; });
